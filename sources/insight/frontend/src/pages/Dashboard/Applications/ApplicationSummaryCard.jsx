...
const ApplicationSummaryCard = ({ dashboardData, loading }) => {
    ...
    if (loading) {
        return (
            <div className="bg-slate-900 rounded-3xl p-6 shadow-xl border border-white/5 animate-pulse min-h-[400px]">
                <div className="h-4 w-32 bg-slate-800 rounded mb-8"></div>
                <div className="flex justify-center mb-8">
                    <div className="w-40 h-40 rounded-full bg-slate-800"></div>
                </div>
                <div className="space-y-4">
                    {[1, 2, 3].map(i => <div key={i} className="h-4 w-full bg-slate-800 rounded"></div>)}
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="bg-slate-900 rounded-[2rem] p-6 shadow-2xl border border-white/5 relative overflow-hidden group hover:border-indigo-500/20 transition-all duration-500 flex flex-col h-full">
            ...
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-sm font-black text-slate-400 uppercase tracking-widest">Applications</h2>
                        <div className="text-xl font-black text-white tracking-tighter mt-0.5">
                            {formatBytes(totalUsage)}
                            <span className="text-[10px] text-slate-500 ml-2 font-bold uppercase tracking-widest">Total 24h</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col lg:flex-row items-center gap-8 mb-4 min-w-0 w-full">
                ...
                <div className="w-full lg:w-1/2 h-[200px] min-w-0 relative">
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
                        ...
                        <div className="text-base font-black text-white tracking-tight">DPI</div>
                    </div>
                </div>
                    ...
                    {top5.map((cat) => (
                        <div key={cat.id} className="group/item">
                            <div className="flex justify-between items-center mb-1.5">
                                ...
                                <span className="text-[10px] font-mono text-indigo-400 font-bold">{cat.percentage.toFixed(1)}%</span>
                            </div>
                            <div className="w-full h-1 bg-slate-800/50 rounded-full overflow-hidden border border-white/5">
                                <div
            ...
            <div className="mt-auto pt-6 border-t border-white/5">
                <button
                    ...
                    className="flex items-center justify-between w-full h-12 px-6 bg-slate-800/50 hover:bg-indigo-500/10 rounded-2xl border border-white/5 hover:border-indigo-500/30 transition-all group/btn"
                >
                    <span className="text-[10px] font-black text-slate-400 group-hover/btn:text-white uppercase tracking-widest">Explore Full Traffic</span>
                    <ArrowRight size={14} className="text-slate-600 group-hover/btn:translate-x-1 group-hover/btn:text-indigo-400 transition-all" />
                </button>
            </div>
        </div>
    );
